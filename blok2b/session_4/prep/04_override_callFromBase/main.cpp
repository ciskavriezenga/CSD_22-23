...
int main()
{
  ...
  std::cout << "\nCalling the sine.tick method, which is a baseclass method.\n"
    << "Inside this baseclass method, the method calculate will be called."
    << std::endl;
  sine.tick();
   ...
   * _______ * * * DIY * * *  _______
   * - go on, in oscillator.h add the keyword as following:
   * virtual void calculate();
   * now rebuild (make clean & make, 'cause we only touched a .h file)
   * _______ * * * DIY * * *  _______
